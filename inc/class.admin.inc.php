	...
	*  option) any later version.                                              *
	...
	class admin
	{
		...
		function acl($content = null)
		{
			if (is_array($content))
			{
				if ($content['save'] || $content['apply'] || $content['delete'])
				{
					...
					foreach(array_merge($content['nations'],array('***new***')) as $n => $nation)
					{
						...
						$accounts = array_unique(array_merge((array)$data['read'],(array)$data['edit'],(array)$data['athlets']));
			...
			{
				foreach(array('read' => EGW_ACL_READ,'edit' => EGW_ACL_EDIT,'athlets' => EGW_ACL_ADD) as $name => $right)
				{
